# Publishing X402Refunds MCP Server to MCP Directory

Guide for publishing X402Refunds' MCP server to the official MCP directory/registry.
...
The MCP directory is a public registry where MCP-compatible agents can discover and use your server. Publishing X402Refunds there makes it discoverable by default in many MCP clients.
...
1. **MCP Server Running**: Your server must be deployed and accessible
   - Discovery endpoint: `https://api.x402refunds.com/.well-known/mcp.json`
   - Invoke endpoint: `https://api.x402refunds.com/mcp/invoke`
...
```bash
curl https://api.x402refunds.com/.well-known/mcp.json | jq .
```
...
curl -v https://api.x402refunds.com/.well-known/mcp.json

# Test invoke endpoint (will fail auth, but should return 401, not 404)
curl -X POST https://api.x402refunds.com/mcp/invoke \
...
```bash
# Test discovery endpoint
curl https://api.x402refunds.com/.well-known/mcp.json | jq '.tools | length'
# Should return: 8
...
- Discovery: `GET https://api.x402refunds.com/.well-known/mcp.json`
- Invoke: `POST https://api.x402refunds.com/mcp/invoke`
...
3. **X402Refunds Support**: support@x402refunds.com
