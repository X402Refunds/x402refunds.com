...
export default defineSchema({
  ...
  // Codebase RAG - Files indexed for semantic search
  codebaseFiles: defineTable({
    path: v.string(),
    name: v.string(),
    content: v.string(),
    fileType: v.string(), // 'component', 'api', 'test', 'config', 'doc', 'asset'
    sizeBytes: v.number(),
  ...
  // Codebase RAG - Vector embeddings for semantic search
  codebaseEmbeddings: defineTable({
  ...
  // Codebase RAG - File dependencies and relationships
  codebaseDependencies: defineTable({
    fromFileId: v.id("codebaseFiles"),
    toFileId: v.id("codebaseFiles"),
    relationshipType: v.string(), // 'imports', 'exports_to', 'tests', 'uses'
    importName: v.optional(v.string()),
  })
    .index("by_from", ["fromFileId"])
    .index("by_to", ["toFileId"]),

  // Codebase RAG - Available package commands
  codebaseCommands: defineTable({
    command: v.string(),
    script: v.string(),
    package: v.string(), // 'root', 'dashboard', 'convex'
    purpose: v.string(),
    category: v.string(), // 'dev', 'build', 'test', 'deploy', 'utility'
  })
    .index("by_category", ["category"])
    .index("by_package", ["package"]),
});
