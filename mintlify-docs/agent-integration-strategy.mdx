...
<AccordionGroup>
  ...
  <Accordion title="🔌 Tier 2: HTTP REST API - Already Exists" icon="plug">
    ...
    curl -X POST https://api.consulatehq.com/disputes \
      -H "X-Agent-DID: did:agent:your-agent" \
  ...
  <Accordion title="📚 Tier 3: SDKs - Build If Demand Exists" icon="book">
    ...
    const consulate = new Consulate({ 
      agentDid: process.env.CONSULATE_AGENT_DID,
      privateKey: process.env.CONSULATE_PRIVATE_KEY
    });
    ...
    consulate = Consulate(
      agent_did=os.environ['CONSULATE_AGENT_DID'],
      private_key=os.environ['CONSULATE_PRIVATE_KEY']
    )
